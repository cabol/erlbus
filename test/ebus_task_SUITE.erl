...
create_task_in_other_process() ->
  Caller = self(),
  spawn(fun() -> Caller ! ebus_task:async(fun() -> nil end) end),
...
t_async3(_Config) ->
  Task = ebus_task:async(?MODULE, wait_and_send, [self(), done]),
...
t_start2(_Config) ->
  Parent = self(),
  Fun = fun(Atom) -> wait_and_send(Parent, Atom) end,
  {ok, Pid} = ebus_task:start(Fun, [done]),
...
t_start3(_Config) ->
  {ok, Pid} = ebus_task:start(?MODULE, wait_and_send, [self(), done]),
...
t_start_link1(_Config) ->
  Parent = self(),
  Fun = fun() -> wait_and_send(Parent, done) end,
  {ok, Pid} = ebus_task:start_link(Fun),
...
t_start_link2(_Config) ->
  Parent = self(),
  Fun = fun(Atom) -> wait_and_send(Parent, Atom) end,
  {ok, Pid} = ebus_task:start_link(Fun, [done]),
...
t_start_link3(_Config) ->
  {ok, Pid} = ebus_task:start_link(?MODULE, wait_and_send, [self(), done]),
...
t_await_timeout(_Config) ->
  ...
  try ebus_task:await(Task, 0)
  catch
    exit:{timeout, {ebus_task, await, [Task, 0]}} -> ok
  end,
...
t_await_normal(_Config) ->
  Task = ebus_task:async(fun() -> exit(normal) end),

  try ebus_task:await(Task)
  catch
    exit:{normal, {ebus_task, await, [Task, 5000]}} -> ok
  end,
...
t_await_task_throw(_Config) ->
  ...
  Task = ebus_task:async(fun() -> throw(unknown) end),

  try ebus_task:await(Task)
  catch
    exit:{{{nocatch, unknown}, _}, {ebus_task, await, [Task, 5000]}} -> ok
  end,
...
t_await_task_error(_Config) ->
  ...
  try ebus_task:await(Task)
    ...
    exit:{{<<"oops">>, _}, {ebus_task, await, [Task, 5000]}} -> ok
  end,
...
t_await_undef_module_error(_Config) ->
  process_flag(trap_exit, true),
  Task = ebus_task:async(fun module_does_not_exist:undef/0),
...
t_await_undef_fun_error(_Config) ->
  process_flag(trap_exit, true),
  Task = ebus_task:async(fun ?MODULE:undef/0),
...
t_await_task_exit(_Config) ->
  process_flag(trap_exit, true),
  Task = ebus_task:async(fun() -> exit(unknown) end),

  try ebus_task:await(Task)
  catch
    exit:{unknown, {ebus_task, await, [Task, 5000]}} -> ok
  end,
